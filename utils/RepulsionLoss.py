...
def repulsion_loss_torch(pbox, gtbox, deta=0.5, pnms=0.1, gtnms=0.1, x1x2y1y2=False):
    ...
    device = pbox.device  # Get device from input tensor
    pgiou = torch.from_numpy(pgiou).to(device).detach()
    ppiou = torch.from_numpy(ppiou).to(device).detach()
    # repgt
    ...
    # repbox
    pp_mask = torch.gt(ppiou, pnms)  # 防止nms�?, 因为如果�?,那么上面的for循环就没有意义了 [N x N] error
